...
library OracleEnvelope {
    ...
    function build(
        ...
        return
            build(
                MemoryPtrLib.alloc(sizeOf(floor, ceiling, maxStaleness, feed)),
                floor,
                ceiling,
                maxStaleness,
                feed
    ...
    function build(
        ...
        ptr = ptr.push(floor, 32).push(ceiling, 32).push(maxStaleness, 4).push(
            feed
        );
    ...
    function exec(Context memory, bytes calldata args) internal view {
        ...
        if (block.timestamp > updatedAt + uint256(maxStaleness))
            revert OracleStale(updatedAt);

        uint8 oracleDecimals = oracle.decimals();
        uint256 px = answer.toUint256();
        if (oracleDecimals < DECIMALS)
            px = px * 10 ** (DECIMALS - oracleDecimals);
        else if (oracleDecimals > DECIMALS)
            px = px / 10 ** (oracleDecimals - DECIMALS);
